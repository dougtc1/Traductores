...
class ArbolBin(Expr):
	...
	def imprimir():
		"""CREO QUE ESTO NO FUNCIONA PERO NO ESTOY CLARO"""
		return self.left.get_valor_left() + self.left.get_valor_right()
...
##############################################################################################################################
"""
LA IMPRESION LA MODULARIZARE EN UNA FUNCION APARTE Y QUE TODOS LA LLAMEN AL IMPRIMIR
"""
	def movPreorden():
		...
		if (self.children):
			for child in self.children:
				if (isinstance(child, ArbolInstr)):
					...
					"""	
					CREO QUE NO NECESITAMOS LO SIGUIENTE PORQUE YA TENDRIAMOS EL NODO QUE ES DISTINTO A ARBOL INSTR
